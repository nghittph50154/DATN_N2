import urllib.request
import urllib.parse
import json
import time
import sys

# Prevent Unicode issues in standard output
if sys.stdout.encoding != 'utf-8':
    try: sys.stdout.reconfigure(encoding='utf-8')
    except AttributeError: pass

OVERPASS_URL = 'https://overpass-api.de/api/interpreter'
BOT_HEADERS = {'User-Agent': 'research-bot/1.0'}

# Bounding boxes for NYC boroughs
BOROUGH_BBOX = {
    "Manhattan":     (40.6960, -74.0203, 40.8820, -73.9067),
    "Brooklyn":      (40.5707, -74.0421, 40.7395, -73.8334),
    "Queens":        (40.5431, -73.9625, 40.8007, -73.7004),
    "The Bronx":     (40.7850, -73.9332, 40.9176, -73.7654),
    "Staten Island": (40.4774, -74.2591, 40.6501, -74.0340),
}

queries = {
    "starbucks": """
        (
          node["amenity"="cafe"]["name"~"Starbucks",i]({s},{w},{n},{e});
          way["amenity"="cafe"]["name"~"Starbucks",i]({s},{w},{n},{e});
        );
    """,
    "parks": """
        (
          node["leisure"="park"]({s},{w},{n},{e});
          way["leisure"="park"]({s},{w},{n},{e});
        );
    """,
    "supermarkets": """
        (
          node["shop"="supermarket"]({s},{w},{n},{e});
          way["shop"="supermarket"]({s},{w},{n},{e});
        );
    """,
    "hospitals": """
        (
          node["amenity"="hospital"]({s},{w},{n},{e});
          node["amenity"="clinic"]({s},{w},{n},{e});
          way["amenity"="hospital"]({s},{w},{n},{e});
          way["amenity"="clinic"]({s},{w},{n},{e});
        );
    """,
    "ev_charging": """
        (
          node["amenity"="charging_station"]({s},{w},{n},{e});
          way["amenity"="charging_station"]({s},{w},{n},{e});
        );
    """
}

results = {boro: {} for boro in BOROUGH_BBOX}

for boro, (s, w, n, e) in BOROUGH_BBOX.items():
    print(f"\n--- Fetching data for {boro} ---")
    for key, q_template in queries.items():
        print(f"  Querying {key}...")
        q_body = q_template.format(s=s, w=w, n=n, e=e)
        query = f"[out:json][timeout:35];\n{q_body}\nout count;"
        
        data = urllib.parse.urlencode({'data': query}).encode('utf-8')
        req  = urllib.request.Request(OVERPASS_URL, data=data, headers=BOT_HEADERS, method='POST')
        
        try:
            with urllib.request.urlopen(req, timeout=45) as r:
                resp  = json.loads(r.read().decode('utf-8'))
                count = resp['elements'][0]['tags']['total']
                print(f"    [OK] {key}: {count}")
                results[boro][key] = int(count)
        except Exception as err:
            print(f"    [ERROR] {key}: {err}")
            results[boro][key] = 0
        time.sleep(3) # Be nice to Overpass API

print("\nFinal OSM Data Collected:")
print(json.dumps(results, indent=4, ensure_ascii=False))

# Now write back to nyc_combined_data.json
json_path = "D:/nyc_combined_data.json"
try:
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    
    for item in data:
        name = item["Quận"]
        key_name = name
        if name == "Bronx":
            key_name = "The Bronx"
        elif name == "The Bronx":
            key_name = "The Bronx"
            
        boro_data = results.get(key_name, {})
        item["Số_cửa_hàng_Starbucks"] = boro_data.get("starbucks", 0)
        item["Số_công_viên"] = boro_data.get("parks", 0)
        item["Số_siêu_thị"] = boro_data.get("supermarkets", 0)
        item["Số_bệnh_viện_phòng_khám"] = boro_data.get("hospitals", 0)
        item["Số_trạm_sạc_xe_điện"] = boro_data.get("ev_charging", 0)
        
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)
    print("\nSuccessfully updated nyc_combined_data.json with all new features!")
except Exception as file_err:
    print(f"Error updating JSON: {file_err}")
